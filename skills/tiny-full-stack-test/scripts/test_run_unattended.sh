...
runner="$root/skills/tiny-full-stack-test/scripts/run-unattended.sh"
tmp=$(mktemp -d)
...
#!/bin/sh
printf '%s %s\n' "$(basename "$0")" "$*" >> "$TINYHOST_UNATTENDED_TEST_LOG"
exit 0
EOF
...
cat > "$tmp/bin/go" <<'EOF'
#!/bin/sh
printf 'go %s\n' "$*" >> "$TINYHOST_UNATTENDED_TEST_LOG"
case " $* " in *' -run TestVPSAcceptance '*) exit "${TINYHOST_UNATTENDED_TEST_LIVE_EXIT:-0}";; esac
case " $* " in *' -skip ^TestVPSAcceptance$ '*) exit "${TINYHOST_UNATTENDED_TEST_OFFLINE_EXIT:-0}";; esac
exit 97
EOF
...
run() {
  PATH="$tmp/bin:$PATH" TINYHOST_UNATTENDED_TEST_LOG="$log" \
  TINYHOST_VPS_UNATTENDED_REPORT_DIR="$tmp/reports" \
  ...
  TINYHOST_RESEND_READER_API_KEY_FILE="$tmp/reader-key" TINYHOST_RESEND_OTP_LEDGER_FILE="$tmp/ledger.json" \
  TINYHOST_VPS_OTP_COMMAND="$root/skills/tiny-full-stack-test/scripts/read-resend-otp.py" "$runner"
}
...
: > "$log"
if env -u TINYHOST_VPS_E2E PATH="$tmp/bin:$PATH" TINYHOST_UNATTENDED_TEST_LOG="$log" TINYHOST_VPS_UNATTENDED_REPORT_DIR="$tmp/reports" "$runner" >/dev/null 2>&1; then
...
: > "$log"
if (
  export TINYHOST_VPS_REUSE=1
  unset TINYHOST_VPS_RELEASE_DIR
  run
) >/dev/null 2>&1; then
