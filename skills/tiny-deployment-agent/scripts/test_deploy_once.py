#!/usr/bin/env python3
"""No-network deterministic tests for the TinyHost deployment-agent wrapper."""
...
class DeploymentAgentTests(unittest.TestCase):
    def setUp(self):
        ...
        self.tiny = self.root / "tiny"
        self.tiny.write_text("#!/bin/sh\nexit 0\n"); self.tiny.chmod(0o700)
        self.app = self.root / "app"; self.app.mkdir()
        (self.app / "tiny.yaml").write_text("name: demo\n"); (self.app / "tiny.yaml").chmod(0o600)
        self.old_env = dict(os.environ)
        os.environ.update({"TINYHOST_RESEND_READER_API_KEY_FILE": str(self.root / "key"),
                           "TINYHOST_RESEND_OTP_LEDGER_FILE": str(self.root / "ledger"),
                           "TINYHOST_VPS_EMAIL_FROM": "tiny@example.test",
    ...
    def run_with(self, responses, login=None):
        ...
        with patch.object(module, "run_json", fake):
            result = module.deploy_once(str(self.tiny), "https://admin.example.test", "dev@example.test", str(self.app), login or (lambda *_: None))
        return result, calls
    ...
    def test_login_failure_blocks_deploy_and_otp_is_not_in_error(self):
        with patch.object(module, "run_json", lambda *_: (1, {"valid": False, "error": {"code": "not_authenticated", "message": "Login required."}})), patch.object(module, "forced_login", lambda *_: (_ for _ in ()).throw(module.DeploymentError("login failure 123456"))):
            ...
            with contextlib.redirect_stderr(stderr):
                self.assertEqual(module.main(["--tiny", str(self.tiny), "--server", "https://admin.example.test", "--deployer-email", "dev@example.test", "--app-dir", str(self.app)]), 1)
            self.assertEqual(stderr.getvalue(), "tiny deployment agent failed\n")

    def test_fake_cli_and_reader_force_login_without_logging_otp(self):
        state, reader = self.root / "state", self.root / "reader"
        self.tiny.write_text("""#!/usr/bin/env python3
import json, os, sys
state = os.environ['FAKE_TINY_STATE']
...
"""); self.tiny.chmod(0o700)
        ...
        old = os.environ.get("FAKE_TINY_STATE")
        os.environ["FAKE_TINY_STATE"] = str(state)
        try:
            ...
            with patch.object(module, "reader_path", lambda: reader), contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                self.assertEqual(module.main(["--tiny", str(self.tiny), "--server", "https://admin.example.test", "--deployer-email", "dev@example.test", "--app-dir", str(self.app)]), 0)
            ...
            if old is None: os.environ.pop("FAKE_TINY_STATE", None)
            else: os.environ["FAKE_TINY_STATE"] = old
...
"""); self.tiny.chmod(0o700)
    ...
    def test_unsafe_non_https_and_missing_inputs_are_denied(self):
        for server in ("http://admin.example.test", "https://admin.example.test/path", "https://admin.example.test:444"):
            with self.assertRaises(module.DeploymentError): module.deploy_once(str(self.tiny), server, "dev@example.test", str(self.app))
        with self.assertRaises(module.DeploymentError): module.deploy_once("tiny", "https://admin.example.test", "dev@example.test", str(self.app))
        with self.assertRaises(module.DeploymentError): module.deploy_once(str(self.tiny), "https://admin.example.test", "dev@example.test", str(self.root / "missing"))

    def test_symlinked_cli_app_and_manifest_are_denied(self):
        linked_tiny = self.root / "linked-tiny"; linked_tiny.symlink_to(self.tiny)
        with self.assertRaises(module.DeploymentError): module.deploy_once(str(linked_tiny), "https://admin.example.test", "dev@example.test", str(self.app))
        linked_app = self.root / "linked-app"; linked_app.symlink_to(self.app, target_is_directory=True)
        with self.assertRaises(module.DeploymentError): module.deploy_once(str(self.tiny), "https://admin.example.test", "dev@example.test", str(linked_app))
        manifest = self.app / "tiny.yaml"; real_manifest = self.app / "tiny-real.yaml"; manifest.rename(real_manifest); manifest.symlink_to(real_manifest)
        with self.assertRaises(module.DeploymentError): module.deploy_once(str(self.tiny), "https://admin.example.test", "dev@example.test", str(self.app))
