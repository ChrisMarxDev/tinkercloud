#!/usr/bin/env python3
"""Read one exact Tinkercloud OTP from Resend for the opt-in VPS acceptance run.
...
def read_key() -> str:
    path = require_private_file(os.environ.get("TINKERCLOUD_RESEND_READER_API_KEY_FILE", ""), "reader key")
    try:
...
def ledger_path() -> Path:
    configured = os.environ.get("TINKERCLOUD_RESEND_OTP_LEDGER_FILE", "")
    if configured:
        if not os.path.isabs(configured):
            fail("consumed ledger is unavailable")
        return Path(configured)
    key_path = Path(os.environ["TINKERCLOUD_RESEND_READER_API_KEY_FILE"])
...
def extract_code(message: Any, email: str, sender: str) -> str:
    ...
    if match is None:
        fail("Resend message does not contain a Tinkercloud OTP")
...
def fetch_json(url: str, key: str, opener: Callable[[urllib.request.Request, int], Any] = open_fixed) -> Any:
    ...
    request = urllib.request.Request(url, headers={"Authorization": "Bearer " + key, "Accept": "application/json", "User-Agent": "Tinkercloud-VPS-E2E/1"})
    try:
...
def timeout_seconds() -> int:
    raw = os.environ.get("TINKERCLOUD_RESEND_OTP_TIMEOUT_SECONDS", "60")
...
def main() -> int:
    try:
        ...
        print("tinkercloud Resend OTP reader failed", file=sys.stderr)
        return 1
