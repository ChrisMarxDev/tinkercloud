---
name: tiny-app-development
description: Build static TinyHost apps using the browser SDK.
---
...
Use `tiny.kv` for current state and live events only to trigger a reread.

## Runnable examples

Start from `examples/sdk-apps/` when the app needs identity, KV, or realtime:

- `shared-checklist` demonstrates all KV operations, bounded cursors,
  optimistic versions, cancellation, and KV change subscriptions;
- `team-pulse` demonstrates custom channel connect, subscribe, publish, status,
  close, and reconnect recovery through a KV reread; and
- `quick-poll` demonstrates `kv.get`, viewer-keyed records, aggregation,
  versioned replacement/deletion, KV change refresh hints, and typed failures.

Run `npm run build` in `examples/sdk-apps/`, then deploy from the selected app
directory. Keep the example's owner-only private policy unless the user names
the exact viewers or domains to add. Do not convert a live hint into durable
state or add an app selector, token, secret, remote API origin, or CDN script.
