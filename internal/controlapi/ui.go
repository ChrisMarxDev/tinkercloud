...
func operatorStartPrompt(actor Actor, platformHost string) string {
	...
	return "Build and deploy a small Tinkercloud app.\n\n" +
		...
		"Follow skills/tinkercloud-deployer/SKILL.md in the repository. Initially ask me only for the deployer email; the bounded OTP exception below is the only later credential question. Run the normal Tinker CLI and deployer skill flow. First check for and reuse the exact server-scoped saved identity for that deployer. Request at most one CLI OTP only when that saved identity is missing, unauthorized, or expired. Only a trusted human-supervised coding agent may ask once for the short-lived emailed OTP after endpoint, email, and manifest validation and the same live CLI process reaches `Code:`. The agent provider may retain the interaction. Immediately submit it only to that same CLI process. Do not deliberately restate or copy it into project files, source, argv, command/tool logs, summaries, or final output. If the CLI OTP fails, stop and report the failure. Never retry the OTP, switch deployer identity, clear, log out, or delete saved CLI authentication, or create another OTP path. Unattended deployment-agent and VPS Resend-reader paths never use agent-interaction OTP. Generate the app, build it, and deploy it with the fewest necessary questions. Never ask for bearer tokens, tokens, secrets, or operator access."
}
