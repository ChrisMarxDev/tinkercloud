...
func TestProtectedStaticPathDeniesBeforeReleaseRead(t *testing.T) {
	...
	h := gateway.Gateway{Config: config.Config{Domain: "apps.tinker.test", SessionCookie: "__Host-tinker_app"}, Apps: appsRepo, Authorizer: appauth.Authorizer{Sessions: store, Policies: policiesStore}}
	inspections := 0
	...
	request := func(host, cookie string) *http.Response {
		...
		r.Host = host
		if cookie != "" {
			r.AddCookie(&http.Cookie{Name: "__Host-tinker_app", Value: cookie})
		}
		resp, e := client.Do(r)
		if e != nil {
			t.Fatal(e)
	...
	for _, tc := range []struct {
		...
		want               int
	}{{"anonymous", "alpha.apps.tinker.test", "", 401}, {"wrong host", "alpha.apps.tinker.test.evil.test", token, 404}, {"wrong app session", "beta.apps.tinker.test", token, 404}} {
	...
	for _, p := range []string{"/_tinker/auth/login", "/_tinker/auth/otp", "/_tinker/auth/verify", "/_tinker/auth/logout", "/_tinker/auth/callback", "/_tinker/not-a-route"} {
		method := http.MethodGet
		if p != "/_tinker/auth/login" && p != "/_tinker/not-a-route" {
	...
	resp := request("alpha.apps.tinker.test", token)
	...
	rangeRequest.Host = "alpha.apps.tinker.test"
	rangeRequest.AddCookie(&http.Cookie{Name: "__Host-tinker_app", Value: token})
	...
	symlinkRequest.Host = "alpha.apps.tinker.test"
	symlinkRequest.AddCookie(&http.Cookie{Name: "__Host-tinker_app", Value: token})
...
func TestHostClassificationFailsClosed(t *testing.T) {
	cfg := config.Config{Domain: "apps.tinker.test"}
	for _, raw := range []string{"alpha.apps.tinker.test", "ALPHA.APPS.TINKER.TEST.", "alpha.apps.tinker.test:443"} {
		h, ok := gateway.ClassifyHost(raw, cfg)
		if !ok || h.Kind != gateway.App || h.Slug != "alpha" {
			t.Fatalf("expected app for %q", raw)
		}
	}
	for _, raw := range []string{"a.b.apps.tinker.test", "-a.apps.tinker.test", "alpha.apps.tinker.test:bad", ""} {
		if _, ok := gateway.ClassifyHost(raw, cfg); ok {
			t.Fatalf("accepted malformed %q", raw)
		}
	}
	for _, raw := range []string{"docs.apps.tinker.test", "auth.apps.tinker.test", "status.apps.tinker.test"} {
		if _, ok := gateway.ClassifyHost(raw, cfg); ok {
			t.Fatalf("accepted reserved app hostname %q", raw)
		}
	}
	for _, raw := range []string{"admin.apps.tinker.test", "ADMIN.APPS.TINKER.TEST"} {
		h, ok := gateway.ClassifyHost(raw, cfg)
		if !ok || h.Kind != gateway.Platform {
			t.Fatalf("admin host was not classified as the platform: %q", raw)
		}
	}
	if h, ok := gateway.ClassifyHost("alpha.apps.tinker.test.evil", cfg); !ok || h.Kind != gateway.Unknown {
...
func TestDirectAppOTPPathsAreReserved(t *testing.T) {
	for _, p := range []string{"/_tinker/auth/otp", "/_tinker/auth/verify"} {
...
func TestAnonymousDocumentNavigationRedirectsOnlyProtectedStatic(t *testing.T) {
	h := gateway.Gateway{
		Config: config.Config{Domain: "apps.tinker.test", SessionCookie: sessions.AppCookieName},
	...
	if w := request(http.MethodGet, "/dashboard?tab=1", "document", "text/html,application/xhtml+xml"); w.Code != http.StatusSeeOther || w.Header().Get("Location") != "/_tinker/auth/login?return=%2Fdashboard%3Ftab%3D1" || strings.Contains(w.Body.String(), "SECRET-RELEASE") {
	...
	for name, tc := range map[string]struct{ method, path, dest, accept string }{
		"api":       {http.MethodGet, "/_tinker/api/v1/me", "document", "text/html"},
		"asset":     {http.MethodGet, "/app.js", "script", "*/*"},
		"range":     {http.MethodGet, "/", "empty", "text/html"},
		"websocket": {http.MethodGet, "/_tinker/ws/v1", "websocket", "*/*"},
	...
	h.ServeHTTP(w, r)
	if w.Code != http.StatusSeeOther || w.Header().Get("Location") != "/_tinker/auth/login?return=%2F" || strings.Contains(w.Body.String(), "SECRET-RELEASE") {
...
func TestSameOriginFailsClosed(t *testing.T) {
	for name, origin := range map[string]string{
		"same":     "https://alpha.apps.tinker.test",
		"cross":    "https://evil.test",
		"http":     "http://alpha.apps.tinker.test",
		"absent":   "",
		"path":     "https://alpha.apps.tinker.test/evil",
		"userinfo": "https://viewer@alpha.apps.tinker.test",
	} {
		t.Run(name, func(t *testing.T) {
			r := httptest.NewRequest(http.MethodPost, "https://alpha.apps.tinker.test/_tinker/auth/logout", nil)
			r.Host = "alpha.apps.tinker.test"
