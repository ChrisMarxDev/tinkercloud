...
func TestSameOriginRequiresExactResolvedHost(t *testing.T) {
	for _, tc := range []struct {
		...
		r := httptest.NewRequest("GET", "https://"+tc.host+"/_tinker/ws/v1", nil)
		r.Host = tc.host
...
func TestCompatibleSDKSubprotocol(t *testing.T) {
	for _, raw := range []string{"", "tinker.sdk.0.1.0.api.1", "tinker.sdk.0.999.12.api.1"} {
		if _, ok := compatibleSubprotocol(raw); !ok {
			t.Fatalf("rejected %q", raw)
		}
	}
	for _, raw := range []string{"tinker.sdk.1.0.0.api.1", "tinker.sdk.0.1.0.api.2", "tinker.sdk.bad.api.1", "other", "tinker.sdk.0.1.0.api.1, other"} {
		if _, ok := compatibleSubprotocol(raw); ok {
