...
func issueAppSessionTx(ctx context.Context, tx *sql.Tx, appID string, viewer identity.Identity, expiry time.Time) (string, sessions.Session, error) {
	...
	_, e := tx.ExecContext(ctx, "INSERT INTO sessions(id,scope,app_id,identity_id,secret_hash,expires_at,created_at) VALUES(?,?,?,?,?,?,?)", v.ID, "app", appID, viewer.ID, h[:], v.ExpiresAt.Format(time.RFC3339Nano), time.Now().UTC().Format(time.RFC3339Nano))
	if e != nil {
		return "", sessions.Session{}, e
	}
	return raw, v, nil
