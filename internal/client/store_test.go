...
const testServer = "https://tiny.pulsebrief.io"
...
func TestFileStoreRejectsUnsafeAndTamperedState(t *testing.T) {
	tests := []struct {
		...
		{"unknown JSON field", func(t *testing.T, _ string, path string) {
			if err := os.WriteFile(path, []byte(`{"version":1,"server":"https://tiny.pulsebrief.io","token":"token","extra":true}`), 0600); err != nil {
				t.Fatal(err)
			}
		}},
		{"duplicate JSON field", func(t *testing.T, _ string, path string) {
			if err := os.WriteFile(path, []byte(`{"version":1,"server":"https://tiny.pulsebrief.io","token":"token","token":"replacement"}`), 0600); err != nil {
				t.Fatal(err)
			}
		}},
...
func TestFileStoreDefaultServerRoundTripAndStrictDecoding(t *testing.T) {
	...
	for _, body := range []string{
		`{"version":1,"server":"https://tiny.pulsebrief.io","extra":true}`,
		`{"version":1,"server":"https://tiny.pulsebrief.io","server":"https://other.example"}`,
		`{"version":1,"server":"http://tiny.pulsebrief.io"}`,
		`{"version":2,"server":"https://tiny.pulsebrief.io"}`,
		`{`,
	} {
