...
func TestActivateDoesNotDependOnLLMConfiguration(t *testing.T) {
	...
	next := Record{Deployment: releases.Deployment{ID: "next", AppID: "app", State: releases.Verified}, OwnerID: "owner", Manifest: releases.Manifest{Name: "app"}}
	...
	if err := service.Activate(context.Background(), Actor{ID: "owner", Active: true}, "next", "request"); err != nil || repo.Current["app"] != "next" {
		t.Fatalf("LLM-independent activation failed: %v %#v", err, repo)
	}
}
