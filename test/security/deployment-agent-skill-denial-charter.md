...
- `TINKERCLOUD_AUTOMATION_RECIPIENT_DOMAIN` is mandatory, single-line, and a
  valid normalized DNS domain. Missing, empty, whitespace-padded, multiline, or
  malformed configuration denies before any CLI/app path check, `whoami`,
  reader/key access, login, deploy, or network action.
