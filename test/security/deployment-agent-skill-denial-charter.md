...
The `tiny-deployment-agent` skill is opt-in local deployer-workstation test
...
- A missing deployer email, app directory, `tiny.yaml`, or HTTPS platform URL
...
- `tiny whoami --json` must contain exactly the expected valid identity before
...
- A missing/invalid saved credential or a verified different identity triggers
  at most one normal `tiny login --force`; it must complete as the exact
...
- Exactly one `tiny deploy --json` may run after identity proof. A nonzero
