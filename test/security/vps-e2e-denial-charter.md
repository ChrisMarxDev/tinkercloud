...
- the root-only local deployer command accepts exactly `tinkercloud deployers
  ...
  mutation;
- no live action occurs without `TINKERCLOUD_VPS_E2E=1` and an acknowledgement
...
- a normal run refuses a host containing Tinkercloud state; reuse requires an
...
- the smoke archive's immutable `tinker.yaml` carries that exact viewer email in
...
- ports 80 and 443 are present and owned by `tinkercloud` after installation;
- any additional non-loopback TCP listener owned by `tinkercloud` fails the
  acceptance run, while an operator-owned listener remains outside Tinkercloud's
  exposure delta; and
