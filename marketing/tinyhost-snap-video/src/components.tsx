...
import {BODY_FONT, DISPLAY_FONT, MONO_FONT, TINY} from "./theme";
...
export const Grain: React.FC<{opacity?: number}> = ({opacity = 0.08}) => {
  return (
    ...
    >
      ...
      <rect width="1920" height="1080" filter="url(#tiny-noise)" opacity="0.52" />
    </svg>
  );
};
  ...
  return (
    <div
      style={{
        ...
        padding: "12px 20px",
        borderRadius: 999,
        color: dark ? "#fffef8" : TINY.ink,
        backgroundColor: dark ? "rgba(255,255,255,0.12)" : TINY.surface,
        border: `2px solid ${
          dark ? "rgba(255,255,255,0.18)" : TINY.line
        }`,
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 34,
        border: `2px solid ${
          dark ? "rgba(255,255,255,0.14)" : TINY.line
        }`,
        backgroundColor: dark ? "#12043f" : TINY.surfaceRaised,
        boxShadow: dark
      ...
      <div
        style={{
          ...
          borderBottom: `2px solid ${
            dark ? "rgba(255,255,255,0.11)" : TINY.line
          }`,
          backgroundColor: dark ? "#19064f" : "#f7f2e9",
        }}
      >
        {[TINY.pink, TINY.yellow, TINY.mint].map((color) => (
          <div
            key={color}
...
export const FileTile: React.FC<{
  ...
  const colors = {
    blue: {bg: TINY.primarySoft, ink: TINY.primary},
    pink: {bg: "#ffe6f2", ink: TINY.pink},
    yellow: {bg: "#fff3bd", ink: "#7e5600"},
    mint: {bg: TINY.mint, ink: TINY.positive},
  } as const;
...
export const LockIcon: React.FC<{
  ...
  color?: string;
  background?: string;
}> = ({size = 100, color = "#fffef8", background = TINY.positive}) => {
  return (
    <div
      style={{
        width: size,
    ...
    >
      <div
        style={{
          ...
          color: dark ? TINY.yellow : TINY.primary,
        }}
      >
        {index}
      </div>
      ...
      {detail ? (
        <div
          style={{
            marginTop: 18,
            maxWidth: 520,
            color: dark ? "rgba(255,255,255,0.68)" : TINY.muted,
            fontSize: 32,
            lineHeight: 1.25,
            fontWeight: 650,
          }}
  ...
  return (
    ...
    >
      <CloudMark
        size={54}
        color={dark ? "#fffef8" : TINY.ink}
        faceColor={dark ? TINY.ink : "#fffbe8"}
        shadowColor={TINY.primary}
      />
      tinyhost
    </div>
