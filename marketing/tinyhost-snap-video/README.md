# TinyHost dynamic product film

A 12.93-second, beat-synchronised Remotion film for TinyHost.

The film turns the product path into one continuous motion arc:

**Create a project → upload it → get a protected live URL**

“Snap” is expressed through acceleration, impact, and settle. It is not shown as
literal on-screen copy.

## Commands

```sh
npm run dev
npm run lint
npm run stills
npm run render
npm run render:no-bgm
```

The main render includes the soundtrack. The no-BGM variant retains the sound
effects for editors who want to lay in different music.

See `DYNAMIC_CUT_SPEC.md` for the creative and timing decisions and
`AUDIO_ATTRIBUTION.md` for the audio sources.
