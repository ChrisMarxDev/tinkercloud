...
On success the requested set is the exact active deployer set: unknown addresses
create active deployers; suspended/revoked deployers reactivate with the same
immutable ID; omitted formerly-active deployers become revoked. Revocation
immediately revokes all CLI/API tokens, invalidates pending CLI OTPs, and
removes dashboard authority on the next request. It preserves the global
browser identity and independently authorized app sessions. Reactivation
invalidates pending CLI OTPs before the address becomes active. Apps,
deployments, policies, access rules, IDs, and other ownership data never
change.
