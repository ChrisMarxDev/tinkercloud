...
The `tiny` binary may coordinate operator-owned host work without becoming a
second server or a remote control plane.

## Boundary

- `tiny host ...` is a workstation-side SSH transport for a fixed command
...
- Host operations accept only an explicit `root@HOST` target. SSH options,
  ...
  targets are invalid.
- OpenSSH performs normal known-host verification. Tiny does not add
...
- `status` and `doctor` call the installed local `tinyhost` operations surface.
