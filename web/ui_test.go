...
func TestEmbeddedDesignSystemIsLocalAccessibleAndTokenized(t *testing.T) {
	...
	for _, required := range []string{
		...
		".tinker-app-more",
		".tinker-insight-layout",
		".tinker-insight-chart",
		".tinker-insight-chart__tooltip",
		".tinker-insight-chart__day:focus",
		"[data-page-level=\"6\"]",
		"[data-visitor-level=\"6\"]",
		"nth-child(-n + 10)",
		".tinker-inset",
...
func TestEmbeddedInteractionsStayLocalAndPresentationOnly(t *testing.T) {
	...
	for _, required := range []string{
		...
		"data-tinker-app-dialog-source",
		"data-tinker-app-dialog-slot",
		"data-tinker-copy-target",
...
func TestShowcaseUsesCanonicalLocalAssetsAndSemanticStates(t *testing.T) {
	...
	for _, required := range []string{
		...
		`class="tinker-app-list" id="showcase-app-list"`,
		`tinker-app-card__identity`,
		`tinker-app-card__title-row`,
		`tinker-app-card__tools`,
		`data-tinker-app-more hidden`,
		`aria-label="more options for payroll-preview"`,
		`data-tinker-app-more-option`,
		`data-tinker-app-dialog="showcase-access-dialog"`,
		`data-tinker-app-dialog-source="showcase-access-dialog"`,
		`data-tinker-app-dialog-slot`,
		...
		`last 30 utc days`,
		`tinker-insight-chart`,
		`tinker-insight-chart__legend-swatch--page`,
		`tinker-insight-chart__legend-swatch--visitors`,
		`tinker-insight-chart__tooltip`,
		`data-page-level="4"`,
		`data-visitor-level="3"`,
		`tabindex="0"`,
		`last 30 utc days raw local insights values`,
		...
		`unattended deployment-agent and vps resend-reader paths never use agent-interaction otp`,
		...
		`data-tinker-copy-feedback="showcase-agent-feedback"`,
		`select the prompt to copy it manually`,
		`removed addresses are signed out`,
		`api keys`,
		`llm chat`,
		`available models are listed live only through verified active api keys`,
		`claude-opus-4-6`,
		`gemini-3.1-flash-lite`,
		`use a custom model identifier`,
		`without waiting for a tinkercloud upgrade`,
		`no models listed`,
		`custom model identifier remains available`,
