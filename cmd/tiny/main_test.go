...
func successfulDeployTransport(t *testing.T, bearer string, archiveNames *[]string, calls *[]string) tokenRoundTrip {
	...
	return tokenRoundTrip(func(r *http.Request) (*http.Response, error) {
		*calls = append(*calls, r.Method+" "+r.URL.Path)
		...
		case r.URL.Host == "demo.tiny.example" && r.URL.Path == "/":
			h := make(http.Header)
...
func TestLoginReusesValidStoredCredentialWithoutPromptOrOTP(t *testing.T) {
	stored := &spyStore{values: map[string]string{"https://tiny.example": "saved-token"}}
	prompt := &loginPrompt{}
	var paths []string
	...
	if code := runWith([]string{"--server", "https://tiny.example", "login"}, &out, &stderr, deps); code != 0 || out.String() != "Logged in as dev@example.test.\n" || stderr.Len() != 0 {
...
func TestLoginUnauthorizedStoredCredentialFallsBackAndReplacesOnlyAfterWhoami(t *testing.T) {
	stored := client.MemoryStore{"https://tiny.example": "old-token"}
	...
	var out, stderr bytes.Buffer
	if code := runWith([]string{"--server", "https://tiny.example", "login"}, &out, &stderr, deps); code != 0 || out.String() != "Logged in as actual@example.test.\n" || stderr.Len() != 0 {
		t.Fatalf("code=%d out=%q err=%q", code, out.String(), stderr.String())
	}
	if prompt.asked != 2 || whoamiCalls != 2 || stored["https://tiny.example"] != "new-token" || strings.Contains(out.String(), "new-token") {
		t.Fatalf("asked=%d whoami=%d store=%q out=%q", prompt.asked, whoamiCalls, stored["https://tiny.example"], out.String())
	}
}

func TestLoginDependencyFailureDoesNotBypassSavedCredentialToOTP(t *testing.T) {
	stored := client.MemoryStore{"https://tiny.example": "saved-token"}
	prompt := &loginPrompt{}
	deps := runnerDeps{store: stored, prompt: prompt, newClient: tokenClient(tokenRoundTrip(func(r *http.Request) (*http.Response, error) {
		return jsonResponse(r, http.StatusServiceUnavailable, `{"error":"dependency unavailable"}`), nil
	}))}
	var out, stderr bytes.Buffer
	if code := runWith([]string{"--json", "--server", "https://tiny.example", "login"}, &out, &stderr, deps); code != 1 || out.String() != "{\"valid\":false,\"error\":{\"code\":\"login_failed\",\"message\":\"Login could not be completed.\"}}\n" || prompt.asked != 0 {
		t.Fatalf("code=%d out=%q err=%q asked=%d", code, out.String(), stderr.String(), prompt.asked)
	}
	if stored["https://tiny.example"] != "saved-token" {
...
func TestLoginRateLimitIsDeterministicAndDoesNotLeakDetails(t *testing.T) {
	...
	var out, stderr bytes.Buffer
	if code := runWith([]string{"--json", "--server", "https://tiny.example", "login"}, &out, &stderr, deps); code != 1 || out.String() != "{\"valid\":false,\"error\":{\"code\":\"rate_limited\",\"message\":\"Too many sign-in attempts. Wait and try again.\"}}\n" {
		t.Fatalf("code=%d out=%q err=%q", code, out.String(), stderr.String())
	}
...
func TestLoginForceSkipsReuseAndDoesNotReplaceCredentialOnFailure(t *testing.T) {
	...
	var out, stderr bytes.Buffer
	if code := runWith([]string{"--server", "https://tiny.example", "login", "--force"}, &out, &stderr, deps); code != 1 || prompt.asked != 2 || stored["https://tiny.example"] != "old-token" || strings.Join(paths, ",") != "GET /api/v1/version,POST /api/v1/auth/otp,POST /api/v1/auth/verify" {
		t.Fatalf("code=%d asked=%d store=%q paths=%v out=%q err=%q", code, prompt.asked, stored["https://tiny.example"], paths, out.String(), stderr.String())
	}
}

func TestLoginForceIsRejectedForNonLoginCommandsBeforeStoreOrNetwork(t *testing.T) {
	called := false
	deps := runnerDeps{store: client.MemoryStore{"https://tiny.example": "saved-token"}, newClient: tokenClient(tokenRoundTrip(func(*http.Request) (*http.Response, error) {
		called = true
		return nil, context.Canceled
	}))}
	var out, stderr bytes.Buffer
	if code := runWith([]string{"--force", "--server", "https://tiny.example", "apps", "list"}, &out, &stderr, deps); code != 2 || stderr.String() != "--force is only valid with login.\n" || called {
...
func TestWhoamiUsesAuthenticationSuccessOutput(t *testing.T) {
	stored := client.MemoryStore{"https://tiny.example": "saved-token"}
	if err := stored.SetDefaultServer("https://tiny.example"); err != nil {
		t.Fatal(err)
	}
	deps := runnerDeps{store: stored, newClient: tokenClient(tokenRoundTrip(func(r *http.Request) (*http.Response, error) {
...
func TestSavedDefaultServerResolvesCommandsAndExplicitServerWins(t *testing.T) {
	stored := client.MemoryStore{"https://tiny.example": "saved", "https://override.example": "override"}
	if err := stored.SetDefaultServer("https://tiny.example"); err != nil {
		t.Fatal(err)
	}
	var seen []string
	...
	if got := strings.Join(seen, ","); got != "tiny.example Bearer saved,override.example Bearer override" {
		t.Fatalf("requests = %q", got)
	}
	if got, _ := stored.DefaultServer(); got != "https://tiny.example" {
...
func TestMissingServerSetupWizardVerifiesCachesAndContinues(t *testing.T) {
	...
	prompt := &loginPrompt{values: []string{"https://tiny.example"}}
	var paths []string
	deps := runnerDeps{store: stored, prompt: prompt, newClient: tokenClient(tokenRoundTrip(func(r *http.Request) (*http.Response, error) {
		paths = append(paths, r.Method+" "+r.URL.Path)
		switch r.URL.Path {
	...
	if prompt.asked != 1 || stored.defaultURL != "https://tiny.example" || strings.Join(paths, ",") != "GET /api/v1/version" {
...
func TestMissingServerSetupWizardDenyPaths(t *testing.T) {
	for _, tt := range []struct {
		...
		{"invalid", "http://tiny.example", nil, nil, false, 0},
		{"incompatible", "https://tiny.example", func(r *http.Request) (*http.Response, error) {
			return jsonResponse(r, http.StatusOK, `{"api_version":2}`), nil
		}, nil, false, 1},
		{"redirect", "https://tiny.example", func(r *http.Request) (*http.Response, error) { return jsonResponse(r, http.StatusFound, ``), nil }, nil, false, 1},
		{"transport", "https://tiny.example", func(*http.Request) (*http.Response, error) { return nil, context.DeadlineExceeded }, nil, false, 1},
		{"store", "https://tiny.example", func(r *http.Request) (*http.Response, error) {
			return jsonResponse(r, http.StatusOK, `{"api_version":1}`), nil
		}, client.ErrStore, false, 1},
		{"json", "https://tiny.example", nil, nil, true, 0},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.json {
				if prompt.asked != 0 || out.String() != "{\"valid\":false,\"error\":{\"code\":\"usage\",\"message\":\"No saved server. Run tiny login --server https://your-tinyhost.example.\"}}\n" {
...
func TestInvalidExplicitServerAndPromptEOFNeverCache(t *testing.T) {
	...
	prompt := &loginPrompt{}
	var out, stderr bytes.Buffer
	if code := runWith([]string{"--server", "http://tiny.example", "apps", "list"}, &out, &stderr, runnerDeps{store: stored, prompt: prompt}); code != 2 || prompt.asked != 0 || stored.defaultURL != "" || stderr.String() != "Server must use HTTPS.\n" {
...
func TestInitGeneratesStrictManifestAndNeverOverwrites(t *testing.T) {
	...
	var out, stderr bytes.Buffer
	if code := runWith([]string{"init", project}, &out, &stderr, runnerDeps{prompt: prompt}); code != 0 || out.String() != "Created "+filepath.Join(project, "tiny.yaml")+".\n" {
		t.Fatalf("code=%d out=%q err=%q", code, out.String(), stderr.String())
	}
	b, err := os.ReadFile(filepath.Join(project, "tiny.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	out.Reset()
	stderr.Reset()
	if code := runWith([]string{"init", project}, &out, &stderr, runnerDeps{prompt: prompt}); code != 1 || stringMustRead(t, filepath.Join(project, "tiny.yaml")) != before {
...
func TestInitAcceptsDisplayedDefaultsAndKeepsOptionalFieldsEmpty(t *testing.T) {
	...
	m, err := releases.ParseManifest([]byte(stringMustRead(t, filepath.Join(project, "tiny.yaml"))))
...
func TestInitNeverFollowsManifestTargetSymlink(t *testing.T) {
	...
	if err := os.Symlink(target, filepath.Join(project, "tiny.yaml")); err != nil {
		t.Fatal(err)
	}
	var out, stderr bytes.Buffer
...
func TestDeployCreatesMissingManifestVerifiesSavedBearerAndArchivesOnlyOutput(t *testing.T) {
	...
	if prompt.asked != 6 || stored.putCalls != 0 || !strings.Contains(out.String(), "Created "+filepath.Join(project, "tiny.yaml")+".") {
		t.Fatalf("asked=%d puts=%d out=%q", prompt.asked, stored.putCalls, out.String())
	}
	if _, err := releases.ParseManifest([]byte(stringMustRead(t, filepath.Join(project, "tiny.yaml")))); err != nil {
		t.Fatal(err)
	}
	if got, want := strings.Join(archiveNames, ","), "index.html,tiny.yaml"; got != want {
...
func TestDeployUnauthorizedBearerUsesOTPAndStoresOnlyVerifiedReplacement(t *testing.T) {
	...
	stored := &spyStore{values: map[string]string{"https://tiny.example": "old-token"}}
	...
	if code := runWith([]string{"--server", "https://tiny.example", "deploy", project}, &out, &stderr, deps); code != 0 || !verifiedNew || stored.values["https://tiny.example"] != "new-token" || stored.putCalls != 1 || prompt.asked != 2 {
		t.Fatalf("code=%d verified=%v stored=%q puts=%d asked=%d out=%q err=%q calls=%v", code, verifiedNew, stored.values["https://tiny.example"], stored.putCalls, prompt.asked, out.String(), stderr.String(), calls)
	}
}
...
func TestDeployPrerequisiteFailuresDoNotReachAppOrDeployment(t *testing.T) {
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			store: &spyStore{values: map[string]string{"https://tiny.example": "saved-token"}},
		...
		t.Run(test.name, func(t *testing.T) {
			var out, stderr bytes.Buffer
			code := runWith([]string{"--server", "https://tiny.example", "deploy", project}, &out, &stderr, runnerDeps{store: test.store, prompt: test.prompt, newClient: tokenClient(test.client)})
...
func TestJSONDeployMissingManifestNeverPromptsWritesOrRequestsNetwork(t *testing.T) {
	...
	called := false
	var out, stderr bytes.Buffer
	code := runWith([]string{"--json", "--server", "https://tiny.example", "deploy", project}, &out, &stderr, runnerDeps{store: stored, prompt: prompt, newClient: tokenClient(tokenRoundTrip(func(*http.Request) (*http.Response, error) {
		called = true
		return nil, context.Canceled
	}))})
	if code != 1 || prompt.asked != 0 || stored.putCalls != 0 || called || out.String() != `{"valid":false,"error":{"code":"manifest_required","message":"Create a valid tiny.yaml with tiny init."}}`+"\n" {
...
func TestDeployExistingManifestSkipsManifestWizard(t *testing.T) {
	...
	var archiveNames, calls []string
	var out, stderr bytes.Buffer
	code := runWith([]string{"--server", "https://tiny.example", "deploy", project}, &out, &stderr, runnerDeps{store: stored, prompt: prompt, newClient: tokenClient(successfulDeployTransport(t, "saved-token", &archiveNames, &calls))})
...
func TestUnknownOrEmptyCommandDoesNotReadDefaultServer(t *testing.T) {
	...
	for _, argv := range [][]string{nil, {"unknown"}} {
		var out, stderr bytes.Buffer
		if code := runWith(argv, &out, &stderr, deps); code != 2 || stderr.String() != "usage: tiny [--json] [--server URL] <command>\n" {
...
func TestLogoutRevokesOnlyCurrentBearerAndHandlesFailures(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			stored := client.MemoryStore{"https://tiny.example": "current-token"}
			if err := stored.SetDefaultServer("https://tiny.example"); err != nil {
				t.Fatal(err)
			}
			calls := 0
			...
			_, exists := stored["https://tiny.example"]
			if exists != tt.keepToken {
				t.Fatalf("token exists=%v, want %v", exists, tt.keepToken)
			}
			if got, err := stored.DefaultServer(); err != nil || got != "https://tiny.example" {
...
func TestLogoutTransportDeleteAndUnsafeCredentialFailuresKeepLocalState(t *testing.T) {
	for _, tt := range []struct {
		...
		{"transport", &spyStore{values: map[string]string{"https://tiny.example": "token"}, defaultURL: "https://tiny.example"}, tokenRoundTrip(func(*http.Request) (*http.Response, error) { return nil, context.DeadlineExceeded })},
		{"local delete", &spyStore{values: map[string]string{"https://tiny.example": "token"}, defaultURL: "https://tiny.example", deleteErr: client.ErrStore}, tokenRoundTrip(func(r *http.Request) (*http.Response, error) { return jsonResponse(r, http.StatusNoContent, ""), nil })},
		{"unsafe credential", &spyStore{values: map[string]string{"https://tiny.example": "token"}, defaultURL: "https://tiny.example", getErr: client.ErrStore}, nil},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := tt.store.values["https://tiny.example"]; got != "token" {
