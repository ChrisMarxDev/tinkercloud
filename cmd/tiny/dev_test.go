...
func TestDevConfigUsesManifestOutputAndProjectLocalState(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(p, "tiny.yaml"), []byte("version: 1\nname: local-checklist\nbuild:\n  output: dist\n"), 0600); err != nil {
		t.Fatal(err)
	}
	...
	if c.AppDir != filepath.Join(p, "dist") || c.StateDir != filepath.Join(p, ".tiny", "local") || c.Slug != "local-checklist" || c.Listen != "127.0.0.1:8788" {
		t.Fatalf("config %#v", c)
	}
}
...
func TestRunWithCapturesDevUsage(t *testing.T) {
	...
	if code != 2 || !strings.Contains(out.String(), "tiny dev is a local server") {
