...
func fakeTinkercloudIdentity(t *testing.T) {
	t.Helper()
	oldDrop, oldPrepare, oldMutation := dropToTinkercloudIdentity, prepareDeployerDatabaseOwnership, deployerMutationRunner
	dropToTinkercloudIdentity = func() error { return nil }
...
func TestDeployersRefreshesOnlyRunningServiceAfterDurableMutation(t *testing.T) {
	...
	err, output = runDeployerCommand(t, "deployers", "authorize", "--config", configPath, "deployer@example.com")
	if err == nil || err.Error() != "tinkercloud: deployer_failed" || output != "" {
...
func TestDeployersDenyMalformedGrammarBeforeMutation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"missing action", []string{"deployers"}, "tinkercloud: invalid_arguments"},
		{"invalid action", []string{"deployers", "wat", "--config", configPath, "a@example.com"}, "tinkercloud: invalid_action"},
		{"extra argument", []string{"deployers", "authorize", "--config", configPath, "a@example.com", "extra"}, "tinkercloud: invalid_arguments"},
		{"misplaced flag", []string{"deployers", "authorize", "a@example.com", "--config", configPath}, "tinkercloud: invalid_arguments"},
		{"unknown flag", []string{"deployers", "authorize", "--unknown=" + secret, "a@example.com"}, "tinkercloud: invalid_arguments"},
		{"invalid email", []string{"deployers", "authorize", "--config", configPath, "invalid"}, "tinkercloud: deployer_failed"},
	} {
...
func TestDeployersRequireRootBeforeParsing(t *testing.T) {
	...
	if err == nil || err.Error() != "tinkercloud: root_required" || output != "" {
...
func TestDeployersIdentityOrDatabaseFailureDoesNotReportSuccess(t *testing.T) {
	oldUID, oldDrop, oldPrepare, oldOpen, oldMutation := effectiveUID, dropToTinkercloudIdentity, prepareDeployerDatabaseOwnership, openDeployerSQLite, deployerMutationRunner
	effectiveUID = func() int { return 0 }
	t.Cleanup(func() {
		effectiveUID = oldUID
		dropToTinkercloudIdentity = oldDrop
	...
	configPath := deployerCommandConfig(t)
	databasePath := filepath.Join(filepath.Dir(configPath), "data", "tinkercloud.db")
	prepareDeployerDatabaseOwnership = func(string) error { return nil }
	dropToTinkercloudIdentity = func() error { return os.ErrPermission }
	deployerMutationRunner = runDeployerMutationInProcess
...
func TestDeployerDatabaseOwnershipHandoffCoversSQLiteSidecarsAndFailsClosed(t *testing.T) {
	oldLookup, oldOwner, oldChown := lookupTinkercloudIdentity, databaseArtifactOwner, chownDeployerDatabaseArtifact
	t.Cleanup(func() {
		lookupTinkercloudIdentity = oldLookup
	...
	lookupTinkercloudIdentity = func() (serviceIdentity, error) { return serviceIdentity{uid: 123, gid: 456}, nil }
