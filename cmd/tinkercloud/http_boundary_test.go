...
func TestPlainHTTPDoesNotReachTinkercloudHandler(t *testing.T) {
	t.Parallel()
	cfg := httpBoundaryConfig(t)
	called := false
	tinkercloud := http.HandlerFunc(func(http.ResponseWriter, *http.Request) { called = true })
	// This mirrors the production order: autocert wraps a plaintext-only
	// fallback, never the Tinkercloud HTTPS gateway.
	cm := certificates.NewAutocert(t.TempDir(), "operator@example.test", cfg.PlatformHost(), nil, func(string) bool { return true })
	h, _ := publicHandlers(cfg, cm, func(host string) bool { return host == cfg.PlatformHost() }, tinkercloud)

	r := httptest.NewRequest(http.MethodGet, "http://admin.apps.example.test/_tinker/auth/login", nil)
	r.Host = cfg.PlatformHost()
	w := httptest.NewRecorder()
	h.ServeHTTP(w, r)

	if called {
		t.Fatal("plaintext request reached Tinkercloud handler")
	}
	if w.Code != http.StatusMovedPermanently {
		t.Fatalf("status = %d, want redirect", w.Code)
	}
	if got, want := w.Header().Get("Location"), "https://admin.apps.example.test/_tinker/auth/login"; got != want {
