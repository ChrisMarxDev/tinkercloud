...
func writeDoctorCredential(t *testing.T, body string, mode os.FileMode) string {
	...
	path := filepath.Join(dir, "tinkercloud.env")
...
func TestDoctorCommandRequiresRootBeforeCredentialRead(t *testing.T) {
	...
	defer out.Close()
	if err := run([]string{"doctor", "--config", filepath.Join(t.TempDir(), "missing.yaml")}, out, out); err == nil || err.Error() != "tinkercloud: root_required" {
