...
func TestBuildHandlerComposesCLILogin(t *testing.T) {
	root := t.TempDir()
	store, err := persistence.OpenSQLite(context.Background(), filepath.Join(root, "tinker.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
	...
	// that real ordering.
	recoveryStore, err := persistence.OpenSQLite(context.Background(), filepath.Join(root, "tinker.db"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlatformVersionTLSHostHarness(t *testing.T) {
	...
	s, e := persistence.OpenSQLite(context.Background(), root+"/tinker.db")
	if e != nil {
		t.Fatal(e)
	}
	defer s.Close()
	...
	cfg := config.Config{Domain: "apps.tinker.test", SessionCookie: "__Host-tinker_app", DataDirectory: root, OTPExpiry: time.Minute, SessionExpiry: time.Hour, Limits: config.ResourceLimits{DiskWarningPercent: 80, DiskStopPercent: 99}}
	...
	os.WriteFile(filepath.Join(project, "tinker.yaml"), manifest, 0600)
	...
	if e != nil || result.Verified() != nil || result.URL != "https://demo.apps.tinker.test/" {
		t.Fatal(result, e)
	}
	...
	// update evidence.
	if e := (update.AnonymousDenyHealth{HTTPHealth: update.HTTPHealth{URL: result.URL + "_tinker/api/v1/capabilities", Client: cl}}).Check(context.Background()); e != nil {
