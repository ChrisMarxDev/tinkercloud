...
func sqliteQuickCheck(ctx context.Context, dataDir string) error {
	db, err := sql.Open("sqlite", "file:"+filepath.Join(dataDir, "tinkercloud.db")+"?mode=ro")
	if err != nil {
		return err
	}
	defer db.Close()
...
func diagnoseWithOptions(parent context.Context, c config.Config, network bool, d diagnosticDeps, credentials doctorCredentials, expectedRollbackSnapshot bool, initStatePath ...string) []operations.Check {
	...
	if network {
		...
		checks = append(checks, checkWith(parent, "dns_wildcard", "resolves", "unavailable", func(ctx context.Context) error {
			return resolved(d.DNSLookup(ctx, "tinkercloud-doctor."+c.AppSuffix()))
		}))
