...
// boundary created by `tinyhost init`. It deliberately has no flag for a
...
func runLLMEnable(args []string, out *os.File) error {
	...
	if rootState != nil {
		return errors.New("tinyhost: llm_root_unavailable")
	}
	if cfg.LLMRootKeyRef != "" {
		if cfg.LLMRootKeyRef != "env:TINYHOST_LLM_ROOT_KEY" || !hasRoot {
			return errors.New("tinyhost: llm_root_unavailable")
		}
	...
	if !hasRoot {
		...
		credentialBytes = append(credentialBytes, []byte("TINYHOST_LLM_ROOT_KEY="+hex.EncodeToString(root)+"\n")...)
		if llmWritePrivate(*credentialPath, credentialBytes) != nil || llmChown(*credentialPath, 0, 0) != nil || llmChmod(*credentialPath, 0600) != nil {
			return errors.New("tinyhost: llm_root_write_failed")
		}
	}
	cfg.LLMRootKeyRef = "env:TINYHOST_LLM_ROOT_KEY"
	rendered, err := cfg.RenderYAML()
	if err != nil || llmWritePrivate(*configPath, rendered) != nil {
		return errors.New("tinyhost: llm_config_write_failed")
	}
	group, err := llmLookupGroup("tinyhost")
	if err != nil {
	...
	if err != nil || gid < 1 || llmChown(*configPath, 0, gid) != nil || llmChmod(*configPath, 0640) != nil {
		return errors.New("tinyhost: config_ownership_failed")
	}
	fmt.Fprintln(out, "LLM capability root configured. Restart tinyhost.service to enable operator connection controls.")
	return nil
}
...
func credentialRootState(path string) (bool, error, []byte) {
	...
	for _, line := range strings.Split(string(b), "\n") {
		...
		if ok && key == "TINYHOST_LLM_ROOT_KEY" {
			count++
