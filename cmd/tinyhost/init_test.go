...
func TestRunPreflightDeniesUnsupportedOSBeforeDependencyChecks(t *testing.T) {
	...
	if err := runPreflight(context.Background(), rt); err == nil || err.Error() != "tinyhost: unsupported_host" {
...
func TestInitCompletesOnlyAfterAllDurableSteps(t *testing.T) {
	...
	credential, err := os.Stat(filepath.Join(root, "etc", "credentials", "tinyhost.env"))
