...
deployer. Its manifest requests `llm.chat`, but that alone does not enable it:
...
Open `https://llm-chat.<domain>` as its allowed viewer. Before the operator
grant, the page must remain usable but show the safe capability-unavailable
...
2. In **LLM chat**, create the bounded profile above using that active
   connection and `gemini-2.5-flash`.
3. In the `llm-chat` app card, open **LLM chat grant**, select the profile, and
   choose **Approve LLM chat grant**.
...
| Allowed viewer on an app with no LLM grant | Discovery omits `llm.chat`; chat returns `403` safe capability-unavailable response. |
| Wrong app/session | `401` or `403`; no `llm.chat`, grant, profile, connection, provider, or provider response. |
| Disable the `llm-chat` grant in its dashboard card | Reloaded discovery omits `llm.chat`; chat is denied on the next request. |
| Disable the Gemini connection | Reloaded discovery omits `llm.chat`; chat is denied on the next request. |

For the two operator mutations, use the dashboard's exact confirmation values
shown beside the controls. Do not guess IDs or send raw form requests. Re-enable
only if a later test deliberately needs a new bounded run; otherwise leave the
grant and connection disabled or revoke/delete the disposable app through its
...
encrypted connection/profile/grant behavior, Gemini adapter conformance, and
