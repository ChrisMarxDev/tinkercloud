...
`tinkercloud deployers authorize --config PATH EMAIL` form reject its config
...
`revoke`. `--config` defaults to `/etc/tinkercloud/config.yaml`; flags after the
...
- The operational shorthand `tinkercloud deployers authorize email@example.com`
