...
The unprivileged Tinkercloud service receives provider credentials through a
root-owned systemd `EnvironmentFile`. A separately invoked `tinkercloud doctor`
...
`tinkercloud doctor` requires a local root caller before it reads credentials. It
uses `/etc/tinkercloud/credentials/tinkercloud.env` by default and permits an
...
- `sudo tinkercloud doctor` works independently of systemd's process environment.
- A non-root shell can still run `tinkercloud status` for offline local facts, but
