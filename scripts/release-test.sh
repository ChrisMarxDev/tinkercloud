...
# artifact.
if TINKERCLOUD_RELEASE_SIGNING_KEY="$tmp/private.pem" "$root/scripts/release-build.sh" 0.1.2 "$tmp/wrong-key" >/dev/null 2>&1; then
...
TINKERCLOUD_RELEASE_PUBLIC_KEY="$tmp/public.pem" \
SOURCE_DATE_EPOCH=0 "$root/scripts/release-build.sh" 0.1.2 "$tmp/release" >/dev/null
TINKERCLOUD_RELEASE_PUBLIC_KEY="$tmp/public.pem" "$root/scripts/release-verify.sh" "$tmp/release" >/dev/null
if "$root/scripts/distribution-prepare.sh" 0.1.2 "$tmp/release" "$tmp/wrong-package" \
  "@wrong/cli" Tinker tinker \
  "https://github.com/ChrisMarxDev/tinkercloud/releases/download/v0.1.2/" >/dev/null 2>&1; then
  echo "distribution accepted a noncanonical npm package" >&2
  exit 1
fi
if "$root/scripts/distribution-prepare.sh" 0.1.2 "$tmp/release" "$tmp/wrong-formula" \
  "@tinkercloud/cli" Wrong tinker \
  "https://github.com/ChrisMarxDev/tinkercloud/releases/download/v0.1.2/" >/dev/null 2>&1; then
  echo "distribution accepted a noncanonical Homebrew formula" >&2
  exit 1
fi
if "$root/scripts/distribution-prepare.sh" 0.1.2 "$tmp/release" "$tmp/wrong-command" \
  "@tinkercloud/cli" Tinker wrong \
  "https://github.com/ChrisMarxDev/tinkercloud/releases/download/v0.1.2/" >/dev/null 2>&1; then
  echo "distribution accepted a noncanonical CLI command" >&2
  exit 1
fi
if "$root/scripts/distribution-prepare.sh" 0.1.2 "$tmp/release" "$tmp/wrong-origin" \
  "@tinkercloud/cli" Tinker tinker \
  "https://github.com/ChrisMarxDev/elsewhere/releases/download/v0.1.2/" >/dev/null 2>&1; then
  echo "distribution accepted a noncanonical release origin" >&2
  exit 1
fi
TINKERCLOUD_RELEASE_PUBLIC_KEY="$tmp/public.pem" \
"$root/scripts/distribution-prepare.sh" 0.1.2 "$tmp/release" "$tmp/distribution" \
  "@tinkercloud/cli" Tinker tinker \
  "https://github.com/ChrisMarxDev/tinkercloud/releases/download/v0.1.2/" >/dev/null
node - "$tmp/distribution" <<'NODE'
const fs = require("node:fs");
const path = require("node:path");
const root = process.argv[2];
const manifest = JSON.parse(fs.readFileSync(path.join(root, "npm", "package.json"), "utf8"));
if (manifest.name !== "@tinkercloud/cli" || manifest.version !== "0.1.2" || manifest.dependencies || manifest.scripts) {
...
if (!formula.includes('version "0.1.2"') || !formula.includes("tinker-darwin-amd64") || !formula.includes("tinker-darwin-arm64")) {
...
TINKERCLOUD_RELEASE_SIGNING_KEY="$tmp/private.pem" \
TINKERCLOUD_RELEASE_PUBLIC_KEY="$tmp/public.pem" \
SOURCE_DATE_EPOCH=0 "$root/scripts/release-build.sh" 0.1.2 "$tmp/release-again" >/dev/null
