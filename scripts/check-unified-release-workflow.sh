...
require '[[ "$CONFIRMATION" == "publish-$CHANNEL-$tag" ]]' 'exact channel confirmation is missing'
require 'git merge-base --is-ancestor "$source_commit" origin/main' 'reviewed main ancestry gate is missing'
require '[[ "$GITHUB_SHA" == "$source_commit" ]]' 'workflow ref must equal the tagged source commit'
require 'gh release view "$tag"' 'existing GitHub release denial is missing'
require 'TINKERCLOUD_REQUIRED_RELEASE_CHANNEL="$required_authority"' 'selected signing-authority policy gate is missing'
require 'package_version=$(node -p' 'SDK package version preflight is missing'
require 'SDK version declarations do not match $VERSION' 'shared release version gate is missing'
...
require 'beta-validation:' 'beta validation path is missing'
require 'stable-validation:' 'stable validation path is missing'
require 'beta-release:' 'direct beta signing path is missing'
require 'stable-release:' 'direct stable signing path is missing'
require 'environment: beta-release' 'direct beta environment is missing'
require 'environment: stable-release' 'direct stable environment is missing'
require 'TINKERCLOUD_BETA_RELEASE_SIGNING_KEY_B64' 'direct beta signing secret is missing'
require 'TINKERCLOUD_RELEASE_SIGNING_KEY_B64' 'direct stable signing secret is missing'
require './scripts/publish-github-release.sh beta' 'direct beta publisher is missing'
require './scripts/publish-github-release.sh stable' 'direct stable publisher is missing'
test "$(grep -c -F -- 'contents: write' "$workflow")" = 2 || { echo 'only direct signing jobs may write contents' >&2; exit 1; }
test "$(grep -c -F -- 'id-token: write' "$workflow")" = 4 || { echo 'only the four npm publisher calls may request OIDC' >&2; exit 1; }
reject '^[[:space:]]+(actions|checks|deployments|discussions|issues|packages|pages|pull-requests|security-events|statuses):[[:space:]]+write' 'release grants an unnecessary write permission'
