...
import { createTinker, TinkerNotAuthorizedError, TinkerVersionIncompatibleError } from "../dist/index.js";

const origin = process.env.TINKER_SDK_CONTRACT_ORIGIN;
const cookie = process.env.TINKER_SDK_CONTRACT_COOKIE;
const appHost = process.env.TINKER_SDK_CONTRACT_HOST;
...
async function networkFetch(input, init = {}, host = appHost, includeCookie = true) {
  ...
  if (body instanceof FormData) {
    const encoded = new Request("http://tinkercloud.invalid", { method: "POST", body });
...
const tinker = createTinker({ fetch: realFetch, origin });
const user = await tinker.user.current();
assert.equal(user.identity.email, "viewer@example.com");
assert.equal(user.app.slug, "alpha");
assert.deepEqual(await tinker.app.info(), { slug: "alpha", features: { kv: true, db: true, blobs: true, realtime: true, llm_chat: false } });
assert.deepEqual((await tinker.capabilities.list()).capabilities.map((c) => c.name), ["user", "kv", "db", "blobs", "live"]);

assert.equal(await tinker.kv.get("items/a"), null);
...
assert.equal((await tinker.blobs.list()).blobs[0].id, uploaded.id);
const bytes = await tinker.blobs.get(uploaded.id);
assert.equal(await bytes.text(), "hello");
assert.deepEqual(await tinker.blobs.delete(uploaded.id), { deleted: true });
assert.equal(await tinker.blobs.get(uploaded.id), null);
const first = await tinker.kv.set("items/a", { value: 1 });
assert.equal(first.version, 1);
await tinker.kv.set("items/b", { value: 2 });
assert.deepEqual((await tinker.kv.get("items/a")).value, { value: 1 });
assert.deepEqual((await tinker.kv.list({ prefix: "items/" })).entries.map((entry) => entry.key), ["items/a", "items/b"]);
assert.deepEqual(await tinker.kv.delete("items/a", { expectedVersion: first.version }), { deleted: true });
assert.equal(await tinker.kv.get("items/a"), null);

// There is no app selector in the SDK. A surplus JavaScript property cannot
// alter the host-derived app either.
await tinker.kv.set("tenant-proof", { app: "alpha" }, { appId: "beta" });
const betaResponse = await networkFetch("/_tinker/api/v1/kv/tenant-proof", {}, appHost.replace("alpha.localhost", "beta.localhost"));
...
const raw = await networkFetch("/_tinker/api/v1/me");
assert.equal(raw.status, 200);

const incompatible = createTinker({
  fetch: (input, init = {}) => {
    const headers = new Headers(init.headers);
    headers.set("X-Tinker-SDK-Version", "9.0.0");
...
await assert.rejects(
  ...
  (error) => error instanceof TinkerVersionIncompatibleError && error.code === "sdk_version_incompatible" && !!error.requestId,
);

const anonymous = createTinker({ fetch: (input, init = {}) => networkFetch(input, init, appHost, false), origin });
await assert.rejects(anonymous.user.current(), TinkerNotAuthorizedError);
