...
const fetch = async (path, init = {}) => {
  ...
  if (path === "/_tiny/api/v1/blobs" && init.method === "POST") {
  ...
  if (path === "/_tiny/api/v1/blobs?limit=10") {
    return new Response(JSON.stringify({
      blobs: [{
        id: "blob_opaque",
        name: "notes.txt",
  ...
  if (path === "/_tiny/api/v1/blobs/blob_opaque") {
  ...
  if (path === "/_tiny/api/v1/blobs/blob_missing") return new Response("", { status: 404 });
  return new Response(JSON.stringify({
    error: { code: "quota_exceeded", message: "full", request_id: "req_safe" },
  }), { status: 429, headers: { "Content-Type": "application/json" } });
};

const tiny = createTiny({ fetch });
...
assert.equal(upload.path, "/_tiny/api/v1/blobs");
...
const list = await tiny.blobs.list({ limit: 10 });
assert.equal(list.blobs[0].contentType, "text/plain");
assert.equal(list.nextCursor, "cursor_opaque");

const data = await tiny.blobs.get("blob_opaque");
assert.ok(data instanceof Blob);
assert.equal(await data.text(), "hello");
assert.equal(await tiny.blobs.get("blob_missing"), null);
assert.deepEqual(await tiny.blobs.delete("blob_opaque"), { deleted: true });

await assert.rejects(
  tiny.blobs.list({ cursor: "server-error" }),
  (error) => error instanceof TinyQuotaExceededError && error.requestId === "req_safe",
);
